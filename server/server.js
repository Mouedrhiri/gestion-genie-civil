...
app.put("/api/projects/:id", (req, res) => {
    ...
    connection.query(
        "UPDATE projects SET client_name = ?, project_object = ?, nature_of_service = ?, service_order_date = ?, execution_delay = ?, amount = ?, status = ? WHERE id = ?",
        [client_name, project_object, nature_of_service, service_order_date, execution_delay, amount, status, projectId],
        (err, results) => {
            if (err) {
...
app.post("/api/materials", (req, res) => {
    const { name, existing_quantity, requested_quantity, unit, unit_price, status } = req.body;

    // Validate received data
    if (!name || existing_quantity == null || requested_quantity == null || !unit || unit_price == null || !status) {
        return res.status(400).json({ error: "All fields are required" });
    }

    console.log("Received data:", req.body);  // Debugging line to see the data structure

    connection.query(
        "INSERT INTO materials (name, existing_quantity, requested_quantity, unit, unit_price, status) VALUES (?, ?, ?, ?, ?, ?)",
        [name, existing_quantity, requested_quantity, unit, unit_price, status],
        (err, results) => {
            if (err) {
...
app.put("/api/materials/:id", (req, res) => {
    ...
    connection.query(
        "UPDATE materials SET name = ?, existing_quantity = ?, requested_quantity = ?, unit = ?, unit_price = ?, status = ? WHERE id = ?",
        [name, existing_quantity, requested_quantity, unit, unit_price, status, materialId],
        (err, results) => {
            if (err) {
...
app.post("/api/tasks", (req, res) => {
    ...
    connection.query(
        "INSERT INTO tasks (project_id, name, description, date, status) VALUES (?, ?, ?, ?, ?)",
...
app.put("/api/tasks/:id", (req, res) => {
    const taskId = req.params.id;
    const { project_id, name, description, date, status } = req.body;

    connection.query(
        "UPDATE tasks SET project_id = ?, name = ?, description = ?, date = ?, status = ? WHERE id = ?",
        [project_id, name, description, date, status, taskId],
        (err, results) => {
            if (err) {
...
app.post("/api/invoices", (req, res) => {
    const { project_id, invoice_number, invoice_object, client_name, gross_total, total_after_deduction, status } = req.body;

    if (!project_id || !invoice_number || !gross_total || !total_after_deduction) {
        return res
            .status(400)
            .json({ error: "Project ID, invoice number, gross total, and total after deduction are required" });
    }

    connection.query(
        "INSERT INTO invoices (project_id, invoice_number, invoice_object, client_name, gross_total, total_after_deduction, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [project_id, invoice_number, invoice_object, client_name, gross_total, total_after_deduction, status],
        (err, results) => {
            if (err) {
                return res
                    .status(500)
...
app.get("/api/invoices", (req, res) => {
    const query = `
        SELECT 
            ...
            invoices.total_after_deduction,
            invoices.status 
        FROM 
            invoices
        JOIN 
...
app.put("/api/invoices/:id", (req, res) => {
    ...
    connection.query(
        "UPDATE invoices SET project_id = ?, invoice_number = ?, invoice_object = ?, client_name = ?, gross_total = ?, total_after_deduction = ?, status = ? WHERE id = ?",
        [project_id, invoice_number, invoice_object, client_name, gross_total, total_after_deduction, status, invoiceId],
        (err, results) => {
            if (err) {
                return res
                    .status(500)
